...
export const generateContentIntermediate = (intermediateSteps: IntermediateStep[] = []): string => {
    const generateDetails = (data: IntermediateStep[]): string => {
        try {
            ...
            return data.map((item) => {
                ...
                // 检测是否是工具调用
                const isToolCall = item.content?.name?.toLowerCase().includes('tool') || 
                                  sanitizedPayload.includes('function_call') || 
                                  sanitizedPayload.includes('tool_call');
                
                let details = `<details id=${currentId} index=${currentIndex}>\n`;
                
                // 为工具调用添加特殊样式
                if (isToolCall) {
                    details += `  <summary id=${currentId} class="tool-call">${item.content?.name || '工具调用'}</summary>\n`;
                    details += `<div class="tool-call-content">\n${sanitizedPayload}\n</div>`;
                } else {
                    details += `  <summary id=${currentId}>${item.content?.name || '思考过程'}</summary>\n`;
                    details += `\n${sanitizedPayload}\n`;
                }
    ...
    try {
        ...
        if (firstStep && firstStep.parent_id) {
            intermediateContent = `<details id=${uuidv4()} index="-1" ><summary id=${firstStep.parent_id}>思考过程</summary>\n${intermediateContent}</details>`;
        }
